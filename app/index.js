...
import { getAppVersion, getProfile, getStudents, handleVersionUpdate, setAppVersion } from '../utils/database';
...
const checkProfileAndVersion = async () => {
  try {
    ...
    // 🔥 ডাটা মাইগ্রেশন সবচেয়ে আগে করুন
    if (savedVersion && savedVersion !== CURRENT_VERSION) {
      console.log(`Updating from version ${savedVersion} to ${CURRENT_VERSION}`);
      const success = await handleVersionUpdate(savedVersion, CURRENT_VERSION);
      if (!success) {
        Alert.alert('Update Error', 'Failed to migrate data. Please contact support.');
        return;
      }
    }
    ...
    Alert.alert('Error', 'Failed to load app. Please reinstall or contact support.');
  }
};
